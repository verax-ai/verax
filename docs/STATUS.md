...
| Isolated install | unreleased | `verax install` (elevated) refuses a Node that a standard user can change, installs `@verax-ai/body` from the npm registry into an administrator-owned directory after `npm audit signatures`, writes `MANIFEST.sha256` from those bytes, puts the state directory under LocalService or the `verax` system user, writes the agent token only in the invoking user's profile, and registers an autostart task or systemd unit that runs that Node. `verax doctor` reports the code directory, a `MANIFEST.sha256` mismatch by file name, a state ACL or mode that is wider than that account, an owner other than Administrators or SYSTEM, and whether autostart is present. `verax approve` with no state directory uses that installed state when it exists. A state directory the process cannot read exits 77 instead of a stack trace. | Protect against an administrator or root. Run on macOS. Stop the operator's own elevated terminal from being driven by the agent if the operator leaves an elevated shell open to it. Separate the body from other Windows services running as LOCAL SERVICE. | `install`, `install-e2e` |
