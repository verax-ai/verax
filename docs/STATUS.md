...
- operator `verax approve <ref>` takes the tenant-scoped ref only; the raw `_ref` a brain printed is not accepted
...
- `audit.explain` is not tenant-scoped: any brain with the read scope can explain another tenant's decision record by ref, and a tenant key is derivable from a public `iss` and `sub`. The record holds hashes, not arguments, but the existence and shape of another tenant's decision are readable
- `deny tenant-mismatch` for another tenant's id and `not-found` for an unused id are different answers, so a brain can test whether an id exists in another tenant
