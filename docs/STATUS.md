...
- F8 — Healthz: carries `ok` only for an unauthenticated probe or a token without `verax:audit`; ledger counts require `verax:audit`; unproven until an unauthenticated probe is watched on a hosted body.
...
- the panel's issuer address is a build-time constant (`VITE_VERAX_ISSUER`, else `http://127.0.0.1:8790`), so `verax desktop --issuer-port` moves the issuer but not the panel's session; reading `authorization_servers` from the resource metadata is the fix and is not built. `verax desktop` does pass its panel port to the issuer allow-list
