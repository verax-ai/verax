...
- S2 — Spend: carries `spend` (cap / payee / daily / currency → always `defer`), authorize-once on retry (`spend-reauth-required`), card CSV reconcile (`matched` / `ghost` / `authorizedUnpaid`); the body does not move money; unproven until a live card statement is compared to a production ledger. `spend-not-wired` remains for `pay` and for `spend` when no spend rule is present (policy fixtures replace the hard deny).
