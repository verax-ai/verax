...
`@verax-ai/inventory` 0.2.0, and in the MCP registry as
...
| Portable verification | 0.1.4 | `verax verify <stateDir>` reads a ledger directory with no body running and nothing on the network, and states four things separately: how many signatures verify, whether the record chain is unbroken, whether each effect is bound to a decision, and **which key** answered. `--key` pins a key the reader holds; `--json` prints the same result for a pipeline; the exit code is 0 only when it verifies. | Say that a key deserves trust. Checking against the key carried in the records proves the files agree with each other, not that the key was ever yours — the answer says so every time rather than hiding it behind a flag. Notice a ledger that was deleted rather than altered: absence is not a chain break, and a directory with no ledger answers `NOT VERIFIED` rather than passing quietly. | `verify-ledger`, `verify-cli` |
...
| Demo | 0.2.1 | `verax demo` starts a loopback body with a temporary ledger, records an allow, a signed refuse and a held spend through the real `/mcp` path, and can leave the directory for `verax verify`. | Mask a data read or reconcile a card statement. Those need a downstream server or a real statement. | `demo` |
