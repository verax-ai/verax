...
- a brain can still test whether a memory id exists in another tenant: the two calls are answered differently, and even under one wording it owns both decision records and can read them apart with `audit.explain`. Closing it means separating what is recorded from what the caller is told, which is not designed yet
- the development issuer's `/authorize` has no `redirect_uri` allow-list, so it hands a code to any address; the code is useless without the verifier, and the endpoint is development only
