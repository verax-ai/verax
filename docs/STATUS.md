...
- G7 — Ledger rotation: designed in `docs/design/ledger-rotation.md`, not implemented. The tree still opens one `decisions.jsonl` and `loadCaches` parses every row. Piece list, chain handoff, durable ref index, and evidence-copy keep/move/drop are on that note. Operator decisions taken 17 Sep 2026: piece bound 50k rows or 200 MB, whichever first; no automatic delete, explicit drop, 13-month default keep window. Unproven until a two-piece boot is measured on the same scripts as the 17 Sep 2026 100k run (`packages/body/perf`).
