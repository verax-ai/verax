...
- ledger rotation carries pieces and a ref index (`docs/design/ledger-rotation.md`); a window that starts inside a piece scans it from the end, and `/api/ledger` without a limit fails above ~150k rows. A multi-body panel stays designed (`docs/design/fleet.md`) and is not implemented
