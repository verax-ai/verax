...
- S1 — Approvals: carries `defer` on `mode: "approve"`, `<stateDir>/approvals.jsonl` (rule sentence + inputs summary + amount/payee), `verax approve` (CLI only; queues when the ledger is locked), a panel pending list with no approve control, an explain `pair` (defer + resolution), lazy `expired`, and `_ref` idempotency (`ref-reuse` / `ref-invalid` under a new nonce). Approved `allow` uses `effectDescriptor` for `effectHash`. Unproven until an operator approves a live hostile-brain call.
